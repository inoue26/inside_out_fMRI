    ...
    for duration in [0.1, 0.15, 0.2, 0.25, 0.3]:
        ...
        for sbj in SUBJECT_IDS:
            for run_id in RUN_IDS:
                ...
                th = saliency_df['max_shift_norm'].quantile(0.5)
                for i, row in saliency_df.iterrows():
                    if row['max_shift_norm'] < th:
                        continue
...
figpath = os.path.join(savefig_dir, 'all_sbj-all_run-{}-median_high.png'.format('_'.join(labels)))
plt.savefig(figpath)
print(figpath)
