...
def feature_prediction(x_train:np.ndarray, y_train:np.ndarray, x_test:np.ndarray, y_test:np.ndarray,
                    n_latent:int=-1, n_components:int=200):
    # x_train: n_samples x n_voxels

    n_train = x_train.shape[0] # n_samples of train data
    src_roi_dim = x_train.shape[1] #  n_voxels of source ROI
    tgt_roi_dim = y_train.shape[1] # n_voxels of target ROI
    n_test = x_test.shape[0] # n_samples of test data
    # print(n_train, n_test, roi_1_dim, roi_2_dim)
    # zero padding
    if n_latent <= 0:
        if src_roi_dim > tgt_roi_dim:
            n_dim_l = tgt_roi_dim
            n_dim_h = src_roi_dim
            n_pad_dim = src_roi_dim - tgt_roi_dim
            # roi1の次元とroi2の次元の差分をpaddingすることで大きさを揃える
            y_train = np.concatenate([y_train.data, np.zeros([n_train, n_pad_dim])], axis=1)
            y_test = np.concatenate([y_test.data, np.zeros([n_test, n_pad_dim])], axis=1)
        elif src_roi_dim < tgt_roi_dim:
            n_dim_l = src_roi_dim
            n_dim_h = tgt_roi_dim
            n_pad_dim = tgt_roi_dim - src_roi_dim
            x_train = np.concatenate([x_train.data, np.zeros([n_train, n_pad_dim])], axis=1)
            x_test = np.concatenate([x_test.data, np.zeros([n_test, n_pad_dim])], axis=1)
        else:
            n_dim_l = src_roi_dim
            n_dim_h = src_roi_dim

        if n_components > n_dim_h:
            n_pad_dim = n_components - n_dim_h
            x_train = np.concatenate([x_train.data, np.zeros([n_train, n_pad_dim])], axis=1)
            y_train = np.concatenate([y_train.data, np.zeros([n_train, n_pad_dim])], axis=1)
            x_test = np.concatenate([x_test.data, np.zeros([n_test, n_pad_dim])], axis=1)
            y_test = np.concatenate([y_test.data, np.zeros([n_test, n_pad_dim])], axis=1)
            n_dim_h = n_components
    else:
        n_dim_l = n_latent
        n_dim_h = n_latent
    train_data = [x_train, y_train] # n_train_samples x n_dim_h
    test_data = [x_test, y_test] # n_test_samples x n_dim_h
    ...
    elif (model_type == 'srm_p') and (n_components > 0):
        srm_p = align(train_data, method='probabilistic_srm', n_features=n_components)
        model = {"method": "probabilistic_srm", "alignment": srm_p, "name": "srm_p", "n_feature": n_components}
    elif (model_type == 'srm_p') and (n_components <= 0):
    ...
    elif (model_type == 'srm_d') and (n_components > 0):
        srm_d = align(train_data, method='deterministic_srm', n_features=n_components)
        model = {"method": "deterministic_srm", "alignment": srm_d, "name": "srm_d", "n_feature": n_components}
    elif (model_type == 'srm_d') and (n_components <= 0):
    ...
    transformed_src_train, transformed_tgt_train = model["alignment"]["transformed"]
    transformed_train = [transformed_src_train, transformed_tgt_train]
    # isc_train, isc_train_mean = calc_irc(model["alignment"]["transformed"], model["n_feature"], viz=viz)
    # import pdb; pdb.set_trace()

    src_p = model["alignment"]['transformation_matrix'][0] # .T # 直交行列の制約を入れているので、転置で逆行列になる
    tgt_p = model["alignment"]['transformation_matrix'][1] # .T
    # import pdb; pdb.set_trace()
    transformed_src_test = np.dot(test_data[0], src_p)
    transformed_tgt_test = np.dot(test_data[1], tgt_p)
    transformed_test = [transformed_src_test, transformed_tgt_test]
    # isc_test, isc_test_mean = calc_irc(transformed_test, model["n_feature"], viz=viz)
    # print('=================================')
    # print('train irc: ', isc_train_mean)
    # print('test irc :', isc_test_mean)
    # print('=================================')

    return transformed_src_train.T, transformed_tgt_train.T, transformed_src_test, transformed_tgt_test, model["n_feature"]
